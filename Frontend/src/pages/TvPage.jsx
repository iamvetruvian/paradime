import React, { useState, useEffect, useRef } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import WatchlistModal from '../components/WatchlistModal';
import useAuth from '../hooks/useAuth';
import '../styles/tv.css';
import '../styles/actions.css';

const BEARER = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJmMjk4OTIwNDBmY2Q5NGE2NjM3M2I4MTcwMjExYWU1NSIsIm5iZiI6MTc2ODUwMzk1NS45MjQsInN1YiI6IjY5NjkzYTkzOGYxN2U1Y2Q1YWE4ODJjYiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.wR22Ym0MqJmDhbQs_-fQXemdHTBr0JeIok07HcYSnzw';
const IMG = 'https://image.tmdb.org/t/p/original/';

function tmdbFetch(url) {
  return fetch(url, { headers: { Authorization: `Bearer ${BEARER}` } }).then(r => r.json());
}

export default function TvPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const user = useAuth();

  const [show, setShow] = useState(null);
  const [credits, setCredits] = useState(null);
  const [backdropSrc, setBackdropSrc] = useState('');
  const [trailerKey, setTrailerKey] = useState(null);
  const [isMuted, setIsMuted] = useState(true);
  const [selectedSeason, setSelectedSeason] = useState(1);
  const [episodes, setEpisodes] = useState([]);
  const [bookmarked, setBookmarked] = useState(false);
  const [watchlistOpen, setWatchlistOpen] = useState(false);
  const iframeRef = useRef(null);

  useEffect(() => {
    if (!id) { navigate('/'); return; }
    Promise.all([
      tmdbFetch(`https://api.themoviedb.org/3/tv/${id}`),
      tmdbFetch(`https://api.themoviedb.org/3/tv/${id}/credits`),
      tmdbFetch(`https://api.themoviedb.org/3/tv/${id}/videos`),
    ]).then(([s, c, v]) => {
      setShow(s);
      setCredits(c);
      if (s.backdrop_path) {
        setBackdropSrc(`https://image.tmdb.org/t/p/w300${s.backdrop_path}`);
        const hd = new Image();
        hd.src = `https://image.tmdb.org/t/p/original${s.backdrop_path}`;
        hd.onload = () => setBackdropSrc(hd.src);
      }
      const trailer = v.results?.find(v => v.type === 'Trailer' && v.site === 'YouTube');
      if (trailer) setTrailerKey(trailer.key);
      const firstSeason = s.seasons?.find(s => s.season_number !== 0)?.season_number || 1;
      setSelectedSeason(firstSeason);
    }).catch(console.error);
  }, [id]);

  useEffect(() => {
    if (!selectedSeason) return;
    tmdbFetch(`https://api.themoviedb.org/3/tv/${id}/season/${selectedSeason}`)
      .then(data => setEpisodes(data.episodes || []))
      .catch(console.error);
  }, [id, selectedSeason]);

  useEffect(() => {
    if (!user?.id) return;
    fetch('/api/bookmarks')
      .then(r => r.json())
      .then(bm => setBookmarked(bm.some(b => b.tmdbId == id && b.mediaType === 'tv')))
      .catch(() => {});
  }, [user, id]);

  const toggleBookmark = async () => {
    if (!user) { window.location.href = '/auth/google'; return; }
    const next = !bookmarked;
    setBookmarked(next);
    try {
      await fetch('/api/bookmarks', {
        method: next ? 'POST' : 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tmdbId: id, mediaType: 'tv' }),
      });
    } catch (e) { setBookmarked(!next); }
  };

  const toggleMute = () => {
    const iframe = iframeRef.current;
    if (!iframe) return;
    if (isMuted) {
      iframe.contentWindow.postMessage('{"event":"command","func":"unMute","args":""}', '*');
      iframe.contentWindow.postMessage('{"event":"command","func":"setVolume","args":[100]}', '*');
    } else {
      iframe.contentWindow.postMessage('{"event":"command","func":"mute","args":""}', '*');
    }
    setIsMuted(m => !m);
  };

  if (!show) return <div style={{ background: 'black', minHeight: '100vh', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading...</div>;

  const seasons = show.seasons?.filter(s => s.season_number !== 0) || [];

  return (
    <div style={{ background: 'black', display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <Navbar />
      <div className="tv-info-container">
        <div className="top-banner-section" style={{ overflow: 'hidden' }}>
          <div className="title" id="tv-title" style={{ zIndex: 10 }}>{show.original_name}</div>
          <img className="backdrop-image" id="tv-backdrop" src={backdropSrc} alt="" fetchpriority="high" />
          <div className="action-buttons-container" style={{ position: 'absolute', bottom: 20, right: 20, display: 'flex', gap: 10, zIndex: 10 }}>
            <button className={`btn btn-outline icon-btn${bookmarked ? ' active' : ''}`} title="Bookmark" onClick={toggleBookmark}>
              <i className={`fa-${bookmarked ? 'solid' : 'regular'} fa-bookmark`} />
            </button>
            <button className="btn btn-outline icon-btn" title="Add to Watchlist" onClick={() => { if (!user) { window.location.href = '/auth/google'; return; } setWatchlistOpen(true); }}>
              <i className="fa-solid fa-plus" />
            </button>
            {trailerKey && (
              <button className="btn btn-outline icon-btn" title="Mute/Unmute Trailer" onClick={toggleMute}>
                <i className={`fa-solid fa-volume-${isMuted ? 'xmark' : 'high'}`} />
              </button>
            )}
          </div>
          {trailerKey && (
            <iframe
              ref={iframeRef}
              src={`https://www.youtube.com/embed/${trailerKey}?autoplay=1&mute=1&controls=0&disablekb=1&modestbranding=1&rel=0&playsinline=1&enablejsapi=1&loop=1&playlist=${trailerKey}`}
              style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '100vw', height: '56.25vw', minHeight: '100%', minWidth: '177.77vh', zIndex: 1, pointerEvents: 'none', border: 'none', opacity: 0, transition: 'opacity 1.5s' }}
              allow="autoplay; encrypted-media"
              title="Trailer"
              onLoad={(e) => setTimeout(() => { e.target.style.opacity = '1'; }, 1500)}
            />
          )}
        </div>

        <div className="episode-selector-container">
          <div className="episode-selector glass">
            <select
              id="season-select"
              value={selectedSeason}
              onChange={(e) => setSelectedSeason(Number(e.target.value))}
            >
              {seasons.map(s => (
                <option key={s.season_number} value={s.season_number}>Season {s.season_number}</option>
              ))}
            </select>
          </div>
        </div>

        <div className="bottom-part">
          <div className="tv-information-container">
            <div className="over-view">
              <div className="section-title"><div className="bar-style" />Episodes</div>
              <div className="episode-list-container">
                <div className="episode-list glass" id="episode-list">
                  {episodes.map(ep => (
                    <div
                      key={ep.id}
                      className="episode-card"
                      onClick={() => navigate(`/play?type=tv&id=${id}&s=${selectedSeason}&e=${ep.episode_number}`)}
                    >
                      <div className="episode-image">
                        {ep.still_path && (
                          <img className="episode-thumbnail" src={IMG + ep.still_path} loading="lazy" decoding="async" alt="" />
                        )}
                      </div>
                      <div className="episode-details">
                        <div className="episode-title">{ep.name}</div>
                        <div className="episode-synopsis">{ep.overview || 'No overview available'}</div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            </div>

            <div className="Actors">
              <div className="section-title"><div className="bar-style" />Cast</div>
              <div className="cast-card-container" id="cast-container" style={{ border: '1px solid rgba(255,255,255,0.2)', padding: 15, borderRadius: 15 }}>
                {credits?.cast?.slice(0, 12).filter(a => a.profile_path).map(actor => (
                  <div
                    key={actor.id}
                    className="cast-card glass"
                    onClick={() => window.open(`https://www.google.com/search?q=${encodeURIComponent(actor.name)}`, '_blank')}
                  >
                    <img src={actor.profile_path ? IMG + actor.profile_path : ''} loading="lazy" decoding="async" alt={actor.name} />
                    <div className="names">
                      <div className="cast-name">{actor.name}</div>
                      <div className="cast-character">{actor.character}</div>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </div>
        <Footer />
      </div>

      <WatchlistModal
        isOpen={watchlistOpen}
        onClose={() => setWatchlistOpen(false)}
        tmdbId={id}
        mediaType="tv"
        mediaTitle={show.original_name}
      />
    </div>
  );
}
